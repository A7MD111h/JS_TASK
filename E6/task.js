...
const csvToArray = (csvString) => csvString.split('\n').map(row => row.split(','));
...
const csvToObjectArray = (csvString) => {
    const [headers, ...data] = csvToArray(csvString);
...
const extractValuesAtIndexes = (array, indexes) => indexes.map(index => array[index]);
